...
const addClient = async (req,res) =>{
    try {
        const {first_name,last_name,birthday,passport,driver_license,adress,phone} = req.body
        const check = await Client.findOne({first_name,last_name,birthday,passport,driver_license,adress,phone})
        if(check !== null) return res.status(400).send({message:"This information is already added"})
        const data = await Client({first_name,last_name,birthday,passport,driver_license,adress,phone})
        await data.save()
...
const updateClient = async (req,res) =>{
    try {
        ...
        const data = await Client.findById(id)
        if(!data) return res.status(400).send("Id xato kiritlgan")
        await Client.findByIdAndUpdate({_id:id},{first_name,last_name,birthday,passport,driver_license,adress,phone})
