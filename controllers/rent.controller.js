...
const addRent = async(req,res) =>{
    try {
        const {car_id,client_id,from_datetime,to_datetime,rent_status_id,rent_type_id} = req.body
        ...
        let amount;
        if(rent_type_id == 2){
            const total = new Date(to_datetime).getTime() - new Date(from_datetime).getTime()
        ...
        const info = await Rent({car_id,client_id,from_datetime,to_datetime,rent_status_id,rent_type_id,amount})
        await info.save()
...
const updateRent = async(req,res) =>{
    try {
        ...
        await Rent.findByIdAndUpdate({_id:id},{car_id,client_id,from_datetime,to_datetime,rent_status_id,rent_type_id,amount1})
