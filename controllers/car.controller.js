...
const addCar = async (req,res) =>{
    try {
        ...
        const result = await Car({car_number,make,model,year,mileage,price_type_id})
        await result.save()
...
const updateCar = async (req,res) =>{
    try {
        ...
        const idData = await Car.findOne({_id:id})
        if(idData == null) return res.status(400).send("Id is incorrect")
        const data = await Car.findByIdAndUpdate({_id:id},{car_number,make,model,year,mileage,price_type_id})
